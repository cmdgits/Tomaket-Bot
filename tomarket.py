...
def get_access_token(query_data):
    ...
    try:
        ...
        print(f"JSON Decode Error: Truy vấn của bạn sai")
        return None
...
def get_balance(token):
    ...
    try:
        ...
        print(f"JSON Decode Error: Token không hợp lệ")
        return None
    except requests.RequestException as e:
        print(f"Request Error: {e}")
        return None
                ...
                if balance_response is not None:
                    ...
                    if daily_status_code == 400:
                        if daily['message'] == 'already_check':
                            ...
                            print(f"{Fore.GREEN+Style.BRIGHT}[ Daily ]: Đã điểm danh ngày thứ : {day}  | {point} Point{Style.RESET_ALL}", flush=True)
                        else:
                            print(f"{Fore.RED+Style.BRIGHT}[ Daily ]: Thất bại {daily} {Style.RESET_ALL}", flush=True)
                    ...
                    if farming_status_code == 200:
                        ...
                        print(f"{Fore.GREEN+Style.BRIGHT}[ Farming ]: Started. Yêu cầu trong: {int(hours)} Giờ {int(minutes)} phút {Style.RESET_ALL}", flush=True)
                        if datetime.now() > end_time:
                                ...
                                if claim_status_code == 200:
                                    ...
                                    if farming_status_code == 200:
                                        ...
                                        hours, remainder = divmod(remaining_time.total_seconds(), 3600)
                                        minutes, _ = divmod(remainder, 60)
                                        print(f"{Fore.GREEN+Style.BRIGHT}[ Farming ]: Started. Claim in: {int(hours)} Giờ {int(minutes)} phút {Style.RESET_ALL}", flush=True)
                        ...
                        if farming['message'] == 'game already started':
                            ...
                            hours, remainder = divmod(remaining_time.total_seconds(), 3600)
                            minutes, _ = divmod(remainder, 60)
                            print(f"{Fore.CYAN+Style.BRIGHT}[ Farming ]: Already Started. Claim in: {int(hours)} Giờ {int(minutes)} phút {Style.RESET_ALL}", flush=True)
                    ...
                    while tiket > 0:
                        ...
                        if play_status != 200:
                            ...
                            for _ in range(30):
                                print(f"{random.choice([Fore.RED, Fore.GREEN, Fore.YELLOW, Fore.BLUE, Fore.MAGENTA, Fore.CYAN, Fore.WHITE])+Style.BRIGHT}[ Game ]: Đang chơi game, thời gian còn lại {30 - _} giây {Style.RESET_ALL}", end="\r", flush=True)
                                time.sleep(1)
                            print(f"{Fore.YELLOW+Style.BRIGHT}[ Game ]: Trò chơi kết thúc! Claiming..                                       ", end="\r", flush=True)
                            point = random.randint(400, 600)
                            claim, claim_status = claim_game(token, point)
                            if claim_status != 200:
                                print(f"{Fore.RED+Style.BRIGHT}[ Game ]: Không thể yêu cầu điểm trò chơi! {Style.RESET_ALL}", flush=True)
                            else:
                                print(f"{Fore.GREEN+Style.BRIGHT}[ Game ]: Thành công. Lấy {point} Poin {Style.RESET_ALL}                    ", flush=True)

                            tiket -= 1
            print(Fore.BLUE + Style.BRIGHT + f"\n==========TẤT CẢ TÀI KHOẢN ĐÃ ĐƯỢC XỬ LÝ==========\n",  flush=True)    
            for _ in range(1800):
                minutes, seconds = divmod(1800 - _, 60)
                print(f"{random.choice([Fore.RED, Fore.GREEN, Fore.YELLOW, Fore.BLUE, Fore.MAGENTA, Fore.CYAN, Fore.WHITE])+Style.BRIGHT}==== [ Tất cả các tài khoản đã được xử lý, tiếp theo là  {minutes} phút {seconds} giây ] ===={Style.RESET_ALL}", end="\r", flush=True)
                time.sleep(1)         
...
/_______  /|__| (____  /__|   
    ...
    print(Fore.CYAN + Style.BRIGHT + f"Up time bot: {int(days)} ngày, {int(hours)} Giờ, {int(minutes)} phút, {int(seconds)} giây\n\n")
